...
<div class="box">
  ...
  &nbsp&nbsp<a class="btn btn-primary" href="{{ route('mobil.create') }}">Tambah</a>
    <!-- /.box-header -->
    <div class="box-body">
        ...
        <tbody>
          @php $no=1 @endphp
				  ...
				  <tr>
				    ...
				    <td>{{$data->merk}}</td>
				    <td>{{$data->plat_no}}</td>
				    <td>Rp.{{number_format($data->harga_sewamobil,2,',','.')}}</td>
				    <td>
              <form action="{{route('mobil.destroy',$data->id)}}" method="post">
                <input type="hidden" name="_method" value="Delete">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <a data-toggle="tooltip" data-placement="top" title="Edit Data" class="btn btn-success" href="/mobil/{{$data->id}}/edit"><i class="fa fa-edit"></i></a>
                <a data-toggle="tooltip" data-placement="top" title="Detail Mobil" class="btn btn-warning" href="{{url('/detail/'.$data->id)}}"><i class="fa fa-info"></i></a>
